...
включает только самые необходимые пакеты и поддерживает версии **Python 3.10 – 3.15**. Расширенная функциональность активируется через
...
| `tls`        | `curl-cffi >=0.7`                                                                      | `chutils.http` — `TLSSession`, `TLSAsyncClient`; клиенты с TLS/JA3/HTTP2 impersonation для обхода Cloudflare/WAF                                                                                                                                                                                                                                             | ⚡ Автоматический fallback на стандартный HTTP-клиент или `OptionalDependencyError` при явном требовании curl-cffi                   |
...
| `scraping`   | `playwright >=1.49`, `selenium >=4.27`, опционально `browserforge >=1.1` | `chutils.scraping.humanize`, `chutils.scraping.fingerprint` — имитация поведения человека (мышь, клавиатура, паузы), анти-детекция (`apply_antidetect_playwright`, `apply_antidetect_selenium`, `extract_clearance_cookies`), синтез цифровой личности (`FingerprintSynthesizer`, `BrowserForgeProvider`) | ⚡ Встроенный процедурный генератор отпечатков работает автономно (Zero-Dependency); `OptionalDependencyError` при явном вызове отсутствующего движка или browserforge |
| `fingerprint`| `browserforge >=1.1`                                                                   | `chutils.scraping.fingerprint` — генерация статистически достоверных байесовских отпечатков браузеров (`BrowserForgeProvider`, `FingerprintSynthesizer`)                                                                                                                                                                          | ⚡ Автоматический fallback на встроенный детерминированный процедурный генератор отпечатков (`ProceduralFingerprintEngine`)            |
| `camoufox`   | `camoufox >=0.4`                                                                       | `chutils.scraping.camoufox` — фабрика `launch_camoufox` для антидетект-браузера на базе Firefox                                                                                                                                                                                                                                | `OptionalDependencyError` при вызове                                                                                                  |
| `redis`      | `redis >=5.0`                                                                          | `chutils.scraping.concurrency` — `RedisTaskQueue` (распределенная очередь задач)                                                                                                                                                                                                                                                                             | `OptionalDependencyError` при вызове                                                                                                  |
| `qt`         | `PyQt6 >=6.6`                                                                          | `chutils.qt` — `QtLogHandler`, `run_async_task`, `BaseMainWindow`, `BaseDialog`                                                                                                                                                                                                                                                                              | `OptionalDependencyError` при вызове                                                                                                  |
| `qt-pyside`  | `PySide6 >=6.6`                                                                        | `chutils.qt` — `QtLogHandler`, `run_async_task`, `BaseMainWindow`, `BaseDialog` (PySide6 бэкенд)                                                                                                                                                                                                                                                             | `OptionalDependencyError` при вызове                                                                                                  |
...
| `testing`    | `pytest >=9.0`                                                                         | `chutils.testing`, `chutils.scraping.testing` — фикстуры для тестов (`temp_config`, `mock_secret_manager`, `local_test_server`, `live_browser_session` и др.)                                                                                                   | `ImportError` при попытке импорта                                                                                                     |
| `full`       | Практически все пакеты (кроме взаимоисключающих `qt` / `qt-pyside`)                    | Вся базовая и расширенная функциональность библиотеки (без дублирования альтернативных GUI-бэкендов)                                                                                                                                                                                                                                                         | —                                                                                                                                     |

> [!NOTE]
> Группа `chutils[full]` намеренно **не включает** GUI биндинги (`PyQt6` / `PySide6`) и распределенный бэкенд `redis`,
> так как пользователь выбирает конкретную графическую библиотеку или инфра-хранилище под свой стек. Для добавления Qt
> установите `chutils[full,qt]` или `chutils[full,qt-pyside]`.
...
*Последнее обновление карты зависимостей: 2026-09-21 (для версии 3.8.1).*
