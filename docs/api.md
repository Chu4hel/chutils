...
2.  **Значение из конфигурационного файла**: Если `service_name` не передан в конструктор (или передан пустым), `SecretManager` попытается найти его в вашем `config.yml` файле в секции `Secrets`.
    ```yaml
    # config.yml
    Secrets:
