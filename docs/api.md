...
Все детали реализации, приоритеты настроек, форматы переменных окружения (включая разделитель вложенности `__` и автоматический парсинг JSON-списков) и примеры перенесены непосредственно в докстринги модулей и функций.
