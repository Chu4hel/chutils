...
def test_async_logging_integration(reset_chutils_state):
    ...
    logger = setup_logger(name="test_async", use_async=True, force_reconfigure=True)
...
def test_custom_masking_regex(reset_chutils_state):
    ...
    logger = setup_logger(
        name="test_mask_regex", custom_patterns=[r"ID-\d{4}"], force_reconfigure=True
    )
    ...
    record = logging.LogRecord(
        ...
        pathname="test.py",
        lineno=1,
        msg="User identifier is ID-1234",
        args=(),
        exc_info=None,
    )
...
def test_predefined_patterns(reset_chutils_state):
    ...
    record = logging.LogRecord(
        ...
        pathname="test.py",
        lineno=1,
        msg="Contact me at secret@example.com please",
        args=(),
        exc_info=None,
    )
