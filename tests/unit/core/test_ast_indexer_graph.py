...
def test_dependency_graph_tracking(tmp_path):
    ...
    assert any(
        e.source == "chutils/core" and e.target == "chutils/logger" for e in graph
    )
...
def test_dependency_graph_weights(tmp_path):
    ...
    edge = next(
        e
        for e in index.dependency_graph
        if e.source == "chutils/logic" and e.target == "chutils/core"
    )
