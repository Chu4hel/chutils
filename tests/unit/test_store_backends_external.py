...
def test_redis_store_missing_dependency() -> None:
    ...
    with (
        ...
        patch("chutils.store.backends.redis.is_redis_available", return_value=False),
    ):
        store = RedisStore()
...
def test_redis_store_sync_operations() -> None:
    ...
    with (
        ...
        patch("chutils.store.backends.redis.is_redis_available", return_value=True),
    ):
...
async def test_redis_store_async_operations() -> None:
    ...
    with (
        patch.dict(
            "sys.modules", {"redis": mock_redis, "redis.asyncio": mock_redis.asyncio}
        ),
        patch("chutils.store.backends.redis.is_redis_available", return_value=True),
    ):
        store = RedisStore(url="redis://localhost:6379/0")
...
def test_memcached_store_missing_dependency() -> None:
    ...
    with patch(
        "chutils.store.backends.memcached.is_pymemcache_available", return_value=False
    ):
...
def test_memcached_store_sync_operations() -> None:
    ...
    with (
        ...
        patch(
            "chutils.store.backends.memcached.is_pymemcache_available",
            return_value=True,
        ),
    ):
