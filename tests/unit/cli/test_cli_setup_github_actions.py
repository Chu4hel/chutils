...
def test_cli_setup_github_actions_no_interactive_default(cli_runner, config_fs):
    ...
    assert fs.exists("src/app/.github/workflows/ci.yml") or fs.exists(
...
def test_cli_setup_github_actions_empty_python(cli_runner, config_fs):
    ...
    assert (
        "Необходимо указать хотя бы одну версию Python" in result.stderr
        or "Необходимо указать хотя бы одну версию Python" in result.stdout
    )
...
def test_cli_setup_github_actions_interactive_cancelled(
    cli_runner, config_fs, monkeypatch
):
    ...
    assert result.exit_code == 1
    assert (
        "Настройка отменена пользователем" in result.stdout
        or "Настройка отменена пользователем" in result.stderr
    )
