...
- **Строгая типизация (Zero-Any Strategy)**:
    - Запрещено использование `Any` в аннотациях типов (используйте конкретные типы, `Protocol`, `TypeVar` или `Union`).
    - Запрещено использование `# type: ignore` без крайне веского обоснования и комментария.
    - Для обобщений в декораторах используйте `ParamSpec` и `TypeVar`.
