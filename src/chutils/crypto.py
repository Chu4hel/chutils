            ...
            with open(temp_out, "wb") as fout:
                ...
                while True:
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Сбой аутентификации чанка #{chunk_index}: неверный ключ или данные повреждены ({exc})") from exc
...
def encrypt_file(
    ...
    if stream:
        return _encrypt_stream_file(fp, seed, out_p, chunk_size=chunk_size)
...
def decrypt_file(
    ...
    if is_stream:
        return _decrypt_stream_file(fp, seed, out_p, raise_on_error=raise_on_error)
