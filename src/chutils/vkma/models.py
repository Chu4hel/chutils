...
class VKMALaunchParams(BaseModel):
    ...
    vk_is_app_user: int = Field(
        default=0, description="Приложение установлено пользователем (1 - да, 0 - нет)"
    )
    vk_are_notifications_enabled: int = Field(
        default=0, description="Разрешены ли уведомления"
    )
    vk_language: str = Field(
        default="ru", description="Язык интерфейса VK (ru, en, uk, etc.)"
    )
    ...
    vk_group_id: int | None = Field(
        default=None, description="ID сообщества, если запущен из группы"
    )
    vk_viewer_group_role: str | None = Field(
        default=None,
        description="Роль пользователя в сообществе (admin, editor, member, none)",
    )
    vk_platform: str | None = Field(
        default=None,
        description="Платформа (desktop_web, mobile_web, mobile_android, mobile_iphone, etc.)",
    )
    vk_is_favorite: int | None = Field(
        default=None, description="Добавлено ли сервис в избранное"
    )
    vk_has_profile_button: int | None = Field(
        default=None, description="Есть ли кнопка в профиле"
    )
    vk_testing_group_id: int | None = Field(
        default=None, description="ID группы тестирования"
    )
