...
class SetupGithubActionsSubCommand(SubCommand):
    ...
    def handle(self, args: argparse.Namespace) -> None:
        ...
        if args.interactive:
            ...
            try:
                python_versions_str = self._ask_str(
                    "Версии Python (через запятую)", python_versions_str
                )
                with_pytest = self._ask_yes_no(
                    "Запускать тесты с pytest?",
                    with_pytest if with_pytest is not None else True,
                )
                with_ruff = self._ask_yes_no(
                    "Запускать линтер ruff?",
                    with_ruff if with_ruff is not None else True,
                )
                with_mypy = self._ask_yes_no(
                    "Запускать статический анализ mypy?",
                    with_mypy if with_mypy is not None else True,
                )
                with_ai_lint = self._ask_yes_no(
                    "Запускать аудит AI-готовности chutils dev ai-lint?",
        ...
        python_versions = [
            v.strip() for v in python_versions_str.split(",") if v.strip()
        ]
        if not python_versions:
            self.err_console.print(
                "[bold red]Ошибка:[/bold red] Необходимо указать хотя бы одну версию Python."
            )
            raise SystemExit(1)
        ...
        try:
            ...
            self.console.print(
                f"[green]Workflow успешно сохранен в {output_file}[/green]"
            )
        except Exception as e:
            self.err_console.print(
                f"[bold red]Ошибка при сохранении файла:[/bold red] {e}"
            )
            raise SystemExit(1)
