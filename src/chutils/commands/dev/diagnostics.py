...
class DiagnosticsSubCommand(SubCommand):
    ...
    def handle(self, args: argparse.Namespace) -> None:
        ...
        if is_rich_enabled():
            ...
            self.console.print(
                f"Общее время выполнения: [cyan]{report.total_time:.4f}[/cyan] сек."
            )
            self.console.print()

            table = Table(
                title="Диагностические проверки",
                show_header=True,
            ...
            print()
            print(
                f"{'Название':<15} | {'Статус':<6} | {'Критичность':<12} | {'Время (с)':<10} | Детали"
            )
            print("-" * 80)
            for result in report.results:
