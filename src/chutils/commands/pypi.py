...
def get_current_index_url() -> str:
    ...
    if sys.platform == "win32":
        ...
        # chutils: ignore[ChutilsIntegrationRule] -- системная переменная Windows USERPROFILE
...
def find_best_mirror(results: list[dict[str, Any]], current_url: str) -> str | None:
    ...
    if (best_speed > 0 and current_speed > 0 and best_speed >= current_speed * 1.5) or (
        best_speed > 0 and current_speed == 0
    ):
        return str(best_result["url"])
