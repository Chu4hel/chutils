...
from chutils.logger import ChutilsLogger, setup_logger
...
def main() -> None:
    ...
    audit_logger: ChutilsLogger = setup_logger(
        "audit", config_section_name="AuditLogger"
    )
    ...
    event_logger: ChutilsLogger = setup_logger(
        "events", config_section_name="EventLogger"
    )
    event_logger.info("Логгер СОБЫТИЙ работает (настроен через EventLogger).")

    print("\n[OK] Логирование выполнено в разные потоки.")
    print(
        "Загляните в папку 'logs/', чтобы увидеть раздельные файлы для каждого логгера."
    )
