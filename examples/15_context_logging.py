...
from chutils import bind_context, clear_context, setup_logger
...
async def main() -> None:
    ...
    await asyncio.gather(
        process_request("REQ-001", "alice"), process_request("REQ-002", "bob")
    )
    ...
    print("\n--- JSON ФОРМАТ ---")
    print(
        "Если включить json_format=True, контекст попадет в отдельное поле 'context':"
    )
    print(
        '{"message": "Начало...", "context": {"req": "REQ-001", "user": "alice"}, ...}'
    )
